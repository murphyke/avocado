...
if not getattr(settings, 'CONCEPT_SEARCH_ENABLED', True) or \
    ...
    warnings.warn('CONCEPT_SEARCH_ENABLED and FIELD_SEARCH_ENABLED have been '
        'deprecated due to changes in Haystack 2.x API. To exclude an index '
        'from being discovered, add the path to the class to EXCLUDED_INDEXES '
        'in the appropriate HAYSTACK_CONNECTIONS entry in settings.')
